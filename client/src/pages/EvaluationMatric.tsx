...
const ContentEvaluation = () => {
  ...
  const { data: healthStatus, isLoading: healthLoading } = useEvaluationHealth();
  ...
  const handleSampleData = () => {
    setFormData({
      ...
      description: "In this video, I'll reveal the exact strategies I used to create viral TikTok content that generated over 10 million views in just 6 months. You'll learn the 5 secret techniques that top creators use but never talk about publicly. From trending hashtags to optimal posting times, I'll show you everything you need to know to blow up on TikTok.",
      tags_cleaned: "viral content, tiktok tips, social media marketing, content creation, viral videos, tiktok algorithm, social media growth, content strategy"
    });
    setErrors({});
  };
  ...
  const getPerformanceCategoryIcon = (category: string) => {
    switch (category.toLowerCase()) {
      case 'viral': case 'excellent': return <Award className="h-4 w-4 text-purple-500" />;
      case 'high': case 'good': return <TrendingUp className="h-4 w-4 text-green-500" />;
      case 'medium': case 'average': return <Activity className="h-4 w-4 text-yellow-500" />;
  ...
  const safeGetRecommendations = () => {
    return analysisResult?.data?.recommendations || [];
  };
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  className={`${errors.description ? "border-red-500" : ""} min-h-[100px]`}
                  rows={4}
                />
                {errors.description && (
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleInputChange("tags_cleaned", e.target.value)}
                ...
                <p className="text-xs text-gray-500">
                  {formData.tags_cleaned.split(',').filter(t => t.trim()).length} tags
                </p>
              </div>
              ...
              <Alert className="border-amber-200 bg-amber-50">
                ...
                <AlertDescription className="text-amber-800 text-xs">
                  <strong>Disclaimer:</strong> AI predictions are estimates. Actual performance may vary due to timing, trends, algorithm changes, and other factors.
                </AlertDescription>
              </Alert>
            ...
            <CardFooter className="bg-gray-50 px-6 py-4 rounded-b-lg">
              <div className="flex flex-col w-full space-y-3">
                <div className="flex justify-between items-center text-xs text-gray-500">
                  <span>Credits: <strong>{user?.creditBalance || 0}</strong></span>
              ...
              >
                ...
                <Alert className="border-blue-200 bg-blue-50">
                  ...
                  <AlertDescription className="text-blue-800 text-xs">
                    <strong>Results Note:</strong> These are AI estimates - actual performance may differ due to various factors.
                  </AlertDescription>
                </Alert>
                ...
                <Card className="shadow-lg border-2 border-green-100">
                  <CardHeader className="bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-t-lg">
                    <CardTitle className="flex items-center justify-between">
                      ...
                      <Badge className={`${getScoreBadgeColor(analysisResult.data.content_score)} border`}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center p-4 bg-purple-50 rounded-lg">
                        <div className="flex items-center justify-center mb-2">
                          {getPerformanceCategoryIcon(safeGetPerformanceMetrics()?.performance_category || 'unknown')}
                          <span className="ml-2 text-lg font-bold text-purple-600">
                            {safeGetPerformanceMetrics()?.performance_category || 'Unknown'}
                          </span>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 pt-4">
                        <div className="text-center">
                          <div className="flex items-center justify-center mb-1">
                            ...
                            <span className={`font-bold ${getViralPotentialColor(safeGetPerformanceMetrics()?.viral_potential || 'unknown')}`}>
                              {safeGetPerformanceMetrics()?.viral_potential || 'N/A'}
                            </span>
                          </div>
                        ...
                        <div className="text-center">
                          <div className="flex items-center justify-center mb-1">
                            ...
                            <span className="font-bold text-green-600">
                              {(safeGetPerformanceMetrics()?.estimated_engagement || 0).toLocaleString()}
                            </span>
                          </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-3">
                          <h4 className="font-medium text-gray-800 flex items-center">
                            <Badge variant="outline" className="mr-2">Title</Badge>
                          </h4>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-gray-600">Length:</span>
                              <span className={safeGetContentAnalysis()?.title.optimal_length ? 'text-green-600' : 'text-yellow-600'}>
                                {safeGetContentAnalysis()?.title.length || 0} chars
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">Words:</span>
                              <span>{safeGetContentAnalysis()?.title.word_count || 0}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">Has Numbers:</span>
                              <span className={safeGetContentAnalysis()?.title.has_numbers ? 'text-green-600' : 'text-gray-600'}>
                                {safeGetContentAnalysis()?.title.has_numbers ? 'Yes' : 'No'}
                              </span>
                            </div>
                            {safeGetContentAnalysis()?.title.capitalization_score && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Capitalization:</span>
                                <span className={getScoreColor(safeGetContentAnalysis()?.title.capitalization_score || 0)}>
                                  {Math.round(safeGetContentAnalysis()?.title.capitalization_score || 0)}%
                                </span>
                              </div>
                            )}
                          </div>
                        ...
                        <div className="space-y-3">
                          <h4 className="font-medium text-gray-800 flex items-center">
                            <Badge variant="outline" className="mr-2">Description</Badge>
                          </h4>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-gray-600">Length:</span>
                              <span className={safeGetContentAnalysis()?.description.optimal_length ? 'text-green-600' : 'text-yellow-600'}>
                                {safeGetContentAnalysis()?.description.length || 0} chars
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">Words:</span>
                              <span>{safeGetContentAnalysis()?.description.word_count || 0}</span>
                            </div>
                            {safeGetContentAnalysis()?.description.readability_score && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Readability:</span>
                                <span className={getScoreColor(safeGetContentAnalysis()?.description.readability_score || 0)}>
                                  {Math.round(safeGetContentAnalysis()?.description.readability_score || 0)}%
                                </span>
                              </div>
                            )}
                          </div>
                        ...
                        <div className="space-y-3">
                          <h4 className="font-medium text-gray-800 flex items-center">
                            <Badge variant="outline" className="mr-2">Tags</Badge>
                          </h4>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-gray-600">Count:</span>
                              <span className={safeGetContentAnalysis()?.tags.optimal_count ? 'text-green-600' : 'text-yellow-600'}>
                                {safeGetContentAnalysis()?.tags.count || 0}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">Avg Length:</span>
                              <span>{Math.round(safeGetContentAnalysis()?.tags.avg_tag_length || 0)} chars</span>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                {/* Recommendations */}
                {safeGetRecommendations().length > 0 && (
                  <Card className="shadow-lg">
                    <CardHeader>
                      <CardTitle className="flex items-center text-lg">
                    ...
                    <CardContent className="p-6">
                      <div className="space-y-3">
                        {safeGetRecommendations().map((rec: string, index: number) => (
