...
export async function getUserDashboardInsights(userId: string) {
  try {
    ...
    const [titleGenerations, keywordAnalyses, sentimentAnalyses] = await Promise.all([
    ...
    const totalUsage = Object.values(featureBreakdown).reduce((sum, count) => sum + count, 0);
    const mostUsedFeature = Object.entries(featureBreakdown)
      .reduce((a, b) => featureBreakdown[a[0] as keyof typeof featureBreakdown] > featureBreakdown[b[0] as keyof typeof featureBreakdown] ? a : b)[0];
    ...
    return {
      ...
      feature_breakdown: Object.entries(featureBreakdown).map(([feature, count]) => ({
        feature,
        count,
...
export async function getUserContentPerformance(userId: string) {
  try {
    ...
    const totalTitles = titleGenerations.reduce((sum, gen) => sum + gen.titles.length, 0);
    const favoriteTitlesCount = titleGenerations.reduce((sum, gen) => 
      sum + gen.titles.filter(title => title.isFavorite).length, 0);
    ...
    return {
      title_performance: {
        ...
        avg_titles_per_generation: titleGenerations.length > 0 ? Math.round(totalTitles / titleGenerations.length) : 0,
      ...
      sentiment_performance: {
        ...
        avg_sentiment: sentimentAnalyses.length > 0 ? {
          positive: sentimentAnalyses.reduce((sum, s) => sum + s.positive, 0) / sentimentAnalyses.length,
          negative: sentimentAnalyses.reduce((sum, s) => sum + s.negative, 0) / sentimentAnalyses.length,
          neutral: sentimentAnalyses.reduce((sum, s) => sum + s.neutral, 0) / sentimentAnalyses.length
        } : null
      }
    };
...
export async function getUserRecommendations(userId: string) {
  try {
    ...
    if (!userEngagement) {
      return {
        feature_suggestions: ['Try keyword analysis to discover trending topics'],
    ...
    if (userEngagement.sentimentAnalysisCount === 0) {
      recommendations.push('Analyze audience sentiment with Sentiment Analysis');
    }
    ...
    if (userEngagement.titleGenerationCount > userEngagement.keywordAnalysisCount) {
      contentTips.push('Research keywords before generating titles for better results');
    }
    if (userEngagement.keywordAnalysisCount > 0 && userEngagement.evaluationMetricCount === 0) {
      contentTips.push('Use evaluation metrics to test your keywords and titles');
    }

    // Optimization tips
    const totalUsage = userEngagement.keywordAnalysisCount + userEngagement.titleGenerationCount + 
                      userEngagement.sentimentAnalysisCount + userEngagement.evaluationMetricCount;
    
    if (totalUsage < 10) {
      optimizationTips.push('Explore different features to maximize your content strategy');
    }
    if (userEngagement.totalSessions > 0 && totalUsage / userEngagement.totalSessions < 2) {
      optimizationTips.push('Try using multiple features per session for better results');
    }

    return {
      feature_suggestions: recommendations.length > 0 ? recommendations : ['You\'re using all features! Keep experimenting.'],
      content_tips: contentTips.length > 0 ? contentTips : ['Your content workflow looks optimized!'],
      optimization_tips: optimizationTips.length > 0 ? optimizationTips : ['Great job utilizing the platform efficiently!']
    };
...
export async function getUserActivitySummary(userId: string, days: number = 7) {
  try {
    ...
    const dailyActivity = activities.reduce((acc, activity) => {
      ...
      acc[date].total += 1;
      acc[date].by_feature[activity.feature] = (acc[date].by_feature[activity.feature] || 0) + 1;
      return acc;
    ...
    const peakHour = Object.entries(hourlyActivity)
      .reduce((a, b) => hourlyActivity[parseInt(a[0])] > hourlyActivity[parseInt(b[0])] ? a : b)[0];
    ...
    return {
      ...
      most_active_day: Object.values(dailyActivity).reduce((a: any, b: any) => a.total > b.total ? a : b, { total: 0 }),
...
function calculateActivityStreak(activities: any[]): number {
  ...
  activities.sort((a, b) => new Date(b.usedAt).getTime() - new Date(a.usedAt).getTime());
...
async function getPerformanceTrends(userId: string) {
  try {
    ...
    return {
      ...
      trend: weeklyChange > 0 ? 'increasing' : weeklyChange < 0 ? 'decreasing' : 'stable'
    };
