...
import { PrismaClient, PolicyType, FeatureType, ContentType } from "@prisma/client";
import { 
  getDailyInsights, 
...
export const getFeatureUsageStats = async (
  ...
  try {
    ...
    const stats = await withCache(`feature_usage_${period}_${detailed}`, async () => {
      ...
      const basicData = {
        usage: result,
        total_usage: Object.values(result).reduce((sum, count) => sum + count, 0),
        period: period,
        most_popular_feature: Object.entries(result).reduce((a, b) => result[a[0]] > result[b[0]] ? a : b)[0]
      };
      ...
      if (detailed === 'true') {
        ...
        const timeBasedUsage = await getFeatureUsageCountByRange(`${days} days`);
        ...
        const popularKeywords = await getPopularContent(ContentType.KEYWORD, 10);
        ...
        return {
          ...
          detailed_stats: {
            ...
            summary: {
              total_features_used: Object.values(result).reduce((sum, count) => sum + count, 0),
              ...
              system_health: performanceStats.successRate > 95 ? 'Excellent' : 
                            performanceStats.successRate > 85 ? 'Good' : 'Needs Attention'
            }
          }
        };
      }
    ...
    const stats = await withCache(cacheKey, async () => {
      ...
      return {
        ...
        insights: {
          ...
          avg_daily_usage: dailyInsights.length > 0 
            ? Math.round(result.reduce((sum, item) => sum + item.count, 0) / dailyInsights.length)
            : 0,
...
export const getComprehensiveInsights = async (
  ...
  try {
    ...
    const insights = await withCache(`comprehensive_insights_${dayCount}`, async () => {
...
export const getUserEngagementInsights = async (
  ...
  try {
    const insights = await withCache('user_engagement_insights', async () => {
      ...
      // Get top engaged users with feature breakdown
      ...
      return {
        ...
        feature_adoption: featureAdoption.map(item => ({
          feature: item.favoriteFeature,
          user_count: item._count.favoriteFeature,
          adoption_rate: totalUsers > 0 ? (item._count.favoriteFeature / totalUsers) * 100 : 0
        }))
      };
    });
...
function calculateFeatureTrends(dailyInsights: any[]) {
  ...
  return {
    keyword_analysis: {
      ...
      percentage: previous.keywordAnalysisCount > 0 
        ? ((latest.keywordAnalysisCount - previous.keywordAnalysisCount) / previous.keywordAnalysisCount) * 100 
        : 0
    },
    title_generation: {
      change: latest.titleGenerationCount - previous.titleGenerationCount,
      percentage: previous.titleGenerationCount > 0 
        ? ((latest.titleGenerationCount - previous.titleGenerationCount) / previous.titleGenerationCount) * 100 
        : 0
    },
    sentiment_analysis: {
      change: latest.sentimentAnalysisCount - previous.sentimentAnalysisCount,
      percentage: previous.sentimentAnalysisCount > 0 
        ? ((latest.sentimentAnalysisCount - previous.sentimentAnalysisCount) / previous.sentimentAnalysisCount) * 100 
        : 0
    },
    evaluation_metric: {
      change: latest.evaluationMetricCount - previous.evaluationMetricCount,
      percentage: previous.evaluationMetricCount > 0 
        ? ((latest.evaluationMetricCount - previous.evaluationMetricCount) / previous.evaluationMetricCount) * 100 
        : 0
    }
  };
}
...
function calculatePeakUsageInsights(dailyInsights: any[]) {
  ...
  const totalUsage = dailyInsights.reduce((sum, day) => {
    return sum + day.keywordAnalysisCount + day.titleGenerationCount + 
           day.sentimentAnalysisCount + day.evaluationMetricCount;
  }, 0);

  const avgDailyUsage = totalUsage / dailyInsights.length;

  const peakDay = dailyInsights.reduce((peak, day) => {
    const dayTotal = day.keywordAnalysisCount + day.titleGenerationCount + 
                    day.sentimentAnalysisCount + day.evaluationMetricCount;
    const peakTotal = peak.keywordAnalysisCount + peak.titleGenerationCount + 
                     peak.sentimentAnalysisCount + peak.evaluationMetricCount;
  ...
  return {
    ...
    peak_day: {
      date: peakDay.date,
      total_usage: peakDay.keywordAnalysisCount + peakDay.titleGenerationCount + 
                   peakDay.sentimentAnalysisCount + peakDay.evaluationMetricCount
    },
...
function calculateUsageVariance(dailyInsights: any[]) {
  const dailyTotals = dailyInsights.map(day => 
    day.keywordAnalysisCount + day.titleGenerationCount + 
    day.sentimentAnalysisCount + day.evaluationMetricCount
  );
  
  const mean = dailyTotals.reduce((sum, total) => sum + total, 0) / dailyTotals.length;
...
function calculateGrowthRate(dailyInsights: any[]) {
  ...
  const recentAvg = recent.reduce((sum, day) => 
    sum + day.keywordAnalysisCount + day.titleGenerationCount + 
    day.sentimentAnalysisCount + day.evaluationMetricCount, 0) / recent.length;
  
  const olderAvg = older.reduce((sum, day) => 
    sum + day.keywordAnalysisCount + day.titleGenerationCount + 
    day.sentimentAnalysisCount + day.evaluationMetricCount, 0) / older.length;
